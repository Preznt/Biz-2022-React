...
// div box 를 만들고 자체적으로 className 을 생성해서 부착하고
...
const App = () => {
   ...
   *
   * useState() 를 잘못 사용하면
   *    state 변수가 무한반복으로 재 생성되는 현상이 발생하기도 한다
  ...
  const todoDelete = (id) => {
    ...
    // prettier-ignore
    const resultTodoList = todoList.filter((todo) => {
      return todo.id !== id;
    });
  ...
  // input box 에 입력하는 text를 todo state 에 담아라
  // 어떻게 ? => 니맘대로
  ...
  return (
    <>
      <header className="w3-container w3-khaki w3-padding-16 w3-center">
        <h1>나의 홈페이지</h1>
      ...
      <section className="w3-container w3-padding-16">
        <Box className="w3-card-4 w3-round-xlarge">
          ...
          <footer className="w3-container w3-sand w3-center w3-padding-16">
            <address>CopyRight &copy; bjw1403@gmail.com</address>
          </footer>
        </Box>
      </section>
    </>
